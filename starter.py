...
class ParserGUI:
    def __init__(self):
        ...
        self.log_text = tk.Text(self.window, height=500)
    ...
    def search(self, placed_name):
        ...
        for index, cell in enumerate(cells):
            if self.stop_event.is_set():
                break
            if cell['value'] == "":
                sheetsearcher.insert_dates_to_table(cells, placed_name)
                self.stop_search()
                return
            ...
            if sheets_date == formatted_jira_date:
                ...
                try:
                    # Иначе, если даты не совпадают, заменить дату в массиве на дату из Jira
                    self.log(f"{placed_name} | Меняю дату для {cell['value']}")
                    cells[index]['to_date'] = formatted_jira_date
